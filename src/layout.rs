...
impl<const C: usize, const R: usize, const L: usize, T: 'static> Layout<C, R, L, T> {
    ...
    pub fn add_tri_layer(&mut self, activation_layers: (usize, usize), target_layer: usize) {
        self.tri_layers
            ...
            .expect("there should not be more tri-layers than layers in total");
    }
    ...
    pub fn tick(&mut self) -> CustomEvent<T> {
        ...
        // resolve events from the "queued_no_wait" queue
        if let Some(s) = self.queued_no_wait.pop_front() {
            return self.resolve_queued_event(s);
        }
    ...
    pub fn event(&mut self, event: Event) {
        ...
        if releases_existing_state {
            if let Some(queued) = self.queued_no_wait.push_back(event.into()) {
